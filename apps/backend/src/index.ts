...
import { dbMiddleware, sanitizeMiddleware, type Env, type Variables } from "./middleware";
  ...
  async (c) => {
    ...
    try {
      const newStaff = await db
        .insert(staffTable)
        .values(staffData)
        .returning({ id: staffTable.id, user: staffTable.user, createdAt: staffTable.createdAt });
...
app.post("/staff/login", dbMiddleware, sanitizeMiddleware, async (c) => {
  ...
  const isValidPassword = await bcrypt.compare(password, staffMember.passwordHash);
  if (!isValidPassword) {
    return c.json({ error: "Invalid credentials" }, 401);
  }
...
app.post("/participants/login", dbMiddleware, sanitizeMiddleware, async (c) => {
  ...
  const isValidPassword = await bcrypt.compare(password, participantData.passwordHash);
  if (!isValidPassword) {
    return c.json({ error: "Invalid credentials" }, 401);
  }
