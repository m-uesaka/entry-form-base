...
export const validateParticipantForm = (formData: NewParticipantFormData): FormErrors => {
  ...
  if (formData.lastNameKana && !/^[\u3040-\u309F\u30FC]+$/.test(formData.lastNameKana)) {
    errors.lastNameKana = 'ひらがなで入力してください'
  }
  if (formData.firstNameKana && !/^[\u3040-\u309F\u30FC]+$/.test(formData.firstNameKana)) {
...
export const validateField = (field: keyof NewParticipantFormData, value: string, formData: NewParticipantFormData): string | null => {
  switch (field) {
    case 'lastNameKanji':
    ...
    case 'lastNameKana':
      ...
      if (value && !/^[\u3040-\u309F\u30FC]+$/.test(value)) return 'ひらがなで入力してください'
      break
    case 'firstNameKana':
