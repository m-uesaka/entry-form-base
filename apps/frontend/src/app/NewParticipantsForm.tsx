...
async function submitParticipantForm(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    return {
      data,
      errors: { submit: error instanceof Error ? error.message : '登録に失敗しました' },
      isSubmitting: false,
    }
  }
}
...
export default function NewParticipantsForm() {
  const [state, submitAction, isPending] = useActionState(submitParticipantForm, initialState)

  const handleInputChange = (field: keyof NewParticipantFormData, value: string) => {
    // Field-level validation on change
    const fieldError = validateField(field, value, { ...state.data, [field]: value })
      ...
      <form action={submitAction} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                state.errors.lastNameKanji ? 'border-red-500' : 'border-gray-300'
              }`}
              placeholder="田中"
            />
            {state.errors.lastNameKanji && <p className="mt-1 text-sm text-red-500">{state.errors.lastNameKanji}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                state.errors.firstNameKanji ? 'border-red-500' : 'border-gray-300'
              }`}
              placeholder="太郎"
            />
            {state.errors.firstNameKanji && <p className="mt-1 text-sm text-red-500">{state.errors.firstNameKanji}</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="たなか"
            />
            {state.errors.lastNameKana && <p className="mt-1 text-sm text-red-500">{state.errors.lastNameKana}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                state.errors.firstNameKana ? 'border-red-500' : 'border-gray-300'
              }`}
              placeholder="たろう"
            />
            {state.errors.firstNameKana && <p className="mt-1 text-sm text-red-500">{state.errors.firstNameKana}</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            {state.errors.confirmPassword && <p className="mt-1 text-sm text-red-500">{state.errors.confirmPassword}</p>}
          </div>
        </div>
