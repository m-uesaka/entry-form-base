...
describe("form.ts", () => {
  describe("PREFECTURES定数", () => {
    it("47都道府県＋「その他」が定義されていること", () => {
      expect(PREFECTURES).toHaveLength(48);
    });

    it("すべての都道府県＋「その他」が正しく含まれていること", () => {
    ...
    it("最後から2番目に沖縄県が含まれていること", () => {
      expect(PREFECTURES[PREFECTURES.length - 2]).toBe("沖縄県");
    });

    it("その他が最後に含まれていること", () => {
      expect(PREFECTURES[PREFECTURES.length - 1]).toBe("その他");
    });
  });
  ...
  describe("Prefecture型", () => {
    it("Prefecture型がPREFECTURES定数の要素と一致すること", () => {
      ...
      const otherAsPrefecture: Prefecture = "その他";
      ...
      expect(PREFECTURES).toContain(otherAsPrefecture);
    });
  });
  ...
  describe("NewParticipantFormData型", () => {
    ...
    it("必須フィールドがすべて含まれていること", () => {
      ...
      requiredFields.forEach((field) => {
        ...
        expect(typeof formData[field as keyof NewParticipantFormData]).toBe(
          "string",
        );
      });
    });
