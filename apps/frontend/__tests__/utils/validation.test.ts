...
import { validateParticipantForm, validateField } from '../../src/utils/validation'
...
describe('validateParticipantForm', () => {
  ...
  describe('パスワードのバリデーション', () => {
    it('パスワードが10文字未満の場合、エラーが返ること', () => {
      ...
      shortPasswords.forEach(password => {
        const formData = { ...validFormData, password, confirmPassword: password }
        const errors = validateParticipantForm(formData)
    ...
    it('パスワードが10文字以上の場合、パスワード長のエラーが返らないこと', () => {
      const validPasswords = ['1234567890', 'verylongpassword', 'password123456']
  ...
  describe('ひらがなのバリデーション', () => {
    it('名字（ふりがな）がひらがな以外の場合、エラーが返ること', () => {
      const invalidKana = ['ヤマダ', 'yamada', 'YAMADA', '山田', '123', 'やまだカタカナ']
    ...
    it('名前（ふりがな）がひらがな以外の場合、エラーが返ること', () => {
      const invalidKana = ['タロウ', 'tarou', 'TAROU', '太郎', '123', 'たろうカタカナ']
